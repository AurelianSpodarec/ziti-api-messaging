...
export const registerUser = async (data: RegisterData, socket: Socket, onlineUsers: Map<string, string>, userSockets: Map<string, string>): Promise<void> => {
  console.log('\n\x1b[32m%s\x1b[0m', `Registering user: \x1b[34m${data.userId}\x1b[0m with socket id: \x1b[34m${socket.id}\x1b[0m...`)
  ...
  try {
    if (existingSocketId !== undefined && existingSocketId !== socket.id) {
      ...
      console.log('\x1b[34m%s\x1b[0m', `Removed existing socket id: ${existingSocketId}`)
    }
    ...
    console.log('\x1b[32m%s\x1b[0m', `User with ID \x1b[34m${userId}\x1b[0m registered`)
    console.log('\x1b[34m%s\x1b[0m', 'Current online users: ', onlineUsers)
